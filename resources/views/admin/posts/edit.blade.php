    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.posts.update", [$post->id]) }}" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('full_text') ? 'is-invalid' : '' }}" name="full_text" id="full_text">{!! old('full_text', $post->full_text) !!}</textarea>
            ...
            <div class="mb-3">
                ...
                <select class="form-control select2 {{ $errors->has('sport') ? 'is-invalid' : '' }}" name="sport_id" id="sport_id">
                    @foreach($sports as $id => $sport)
                        <option value="{{ $id }}" {{ ($post->sport ? $post->sport->id : old('sport_id')) == $id ? 'selected' : '' }}>{{ $sport }}</option>
                    @endforeach
                </select>
                @if($errors->has('sport'))
                    <div class="invalid-feedback">
            ...
            <div class="mb-3">
                ...
                <select class="form-control select2 {{ $errors->has('event') ? 'is-invalid' : '' }}" name="event_id" id="event_id">
                    @foreach($events as $id => $event)
                        <option value="{{ $id }}" {{ ($post->event ? $post->event->id : old('event_id')) == $id ? 'selected' : '' }}>{{ $event }}</option>
                    @endforeach
                </select>
                @if($errors->has('event'))
                    <div class="invalid-feedback">
...
@if(isset($post) && $post->photo)
      var file = {!! json_encode($post->photo) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, '{{ $post->photo->getUrl('thumb') }}')
